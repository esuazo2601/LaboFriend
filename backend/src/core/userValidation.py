...
oauth2_scheme = OAuth2PasswordBearer(
    tokenUrl="token",
    scopes={
        "admin": "lectura y escritura de todo",
...
def auth_check_permission(
    security_scopes: SecurityScopes, token_data: TokenData, authenticate_value: str
):
...
async def get_current_active_user(
    current_user: Usuario = Security(get_current_user, scopes=["admin"])
):
    return current_user
...
async def get_token(form_data: OAuth2PasswordRequestForm):
    user = await auth_user(
        UsuarioDB(email=form_data.username, password=form_data.password)
    )
    ...
    access_token_jwt = create_token(
        {"user": user.nombre, "sub": user.email, "scopes": scopes}, token_expires
    )
...
async def auth_user(user: UsuarioDB):
    if not user_exist(user.email).data:
        raise CREDENTIAL_EXCEPTION
    get_password_dict = (
        supabase.table("Usuario").select("password").eq("email", user.email).execute()
    )
...
async def create_user(user: UsuarioDB):
    ...
    response = (
        ...
        .insert({"email": user.email, "nombre": user.nombre, "password": hash_password})
        .execute()
    )
    if response.data:
        supabase.table("Estudiante").insert({"email": user.email}).execute()
    return response
...
async def make_admin(
    user: Usuario, security_scopes: SecurityScopes, token: str = Depends(oauth2_scheme)
):
    authenticate_value = auth_scope_value(security_scopes)
    token_data = decode_token(token)
    auth_check_permission(security_scopes, token_data, authenticate_value)
    ...
    if admin_exist(user.email).data:
        raise HTTPException(
            status_code=208, detail="Ya cuenta con permisos de administrador"
        )
    result = supabase.table("Administrador").insert({"email": user.email}).execute()
    if result.data:
...
async def make_assist(
    ...
    if assist_exist(user.email).data:
        raise HTTPException(
            status_code=208, detail="Ya cuenta con permisos de ayudante"
        )
    result = supabase.table("Ayudante").insert({"email": user.email}).execute()
    if result.data:
        return HTTPException(
            status_code=201, detail="Privilegios concedidos con éxito", headers="bearer"
        )
...
async def make_student(
    ...
    if student_exist(user.email).data:
        raise HTTPException(
            status_code=208, detail="Ya cuenta con permisos de estudiante"
        )
    result = supabase.table("Estudiante").insert({"email": user.email}).execute()
    if result.data:
        return HTTPException(
            status_code=201, detail="Privilegios concedidos con éxito", headers="bearer"
        )
