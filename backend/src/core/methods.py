...
async def newblock(bloque: Bloque):
    try:
        print(
            bloque.model_dump()
        )  # Asegúrate de que los datos se imprimen correctamente
        response = (
            supabase.table("Bloque")
            .insert({"hora_inicio": bloque.hora_inicio, "hora_fin": bloque.hora_fin})
            .execute()
        )
        return response
    except Exception as e:
...
async def new_room(sala: Sala):
    try:
        ...
        response = (
            supabase.table("Sala")
            .insert({"capacidad": sala.capacidad, "nombre": sala.nombre})
            .execute()
        )
        return response
    except Exception as e:
...
async def delete_room(sala_nombre: str):
    try:
        response = (
            supabase.table("Sala").select("*").eq("nombre", sala_nombre).execute()
        )
        if response.data:
            ...
            raise HTTPException(
                status_code=404, detail="No se encuentra la sala con este nombre"
            )
    except Exception as e:
        raise HTTPException(
...
async def get_microorg_cin(nombre_cientifico: str):
    response = (
        supabase.table("Microorganismo")
        .select("*")
        .eq("nombre_cientifico", nombre_cientifico)
        .execute()
    )
    return response
...
async def update_microorg(id: int, nuevo: ActualizarMicroorganismo):
    if (
        nuevo.procedencia and nuevo.detalles
    ):  # Si se entregan nueva procedencia y detalles
        response = (
            supabase.table("Microorganismo")
            .update({"detalles": nuevo.detalles, "procedencia": nuevo.procedencia})
            .eq("id", id)
            .execute()
        )
        return response
        ...
        response = (
            supabase.table("Microorganismo")
            .update({"detalles": nuevo.detalles})
            .eq("id", id)
            .execute()
        )
        return response
        ...
        response = (
            supabase.table("Microorganismo")
            .update({"procedencia": nuevo.procedencia})
            .eq("id", id)
            .execute()
        )
        return response
...
async def get_investigacion_nm(name_inv: str):
    response = (
        supabase.table("Investigacion").select("*").eq("titulo", name_inv).execute()
    )
    return response
...
async def get_investigacion_date(fecha: str):
    ...
    response = (
        supabase.table("Investigacion")
        .select("*")
        .filter("fecha", "gte", fecha_inicio.strftime("%Y-%m-%d %H:%M:%S"))
        .filter("fecha", "lt", fecha_fin.strftime("%Y-%m-%d %H:%M:%S"))
        .execute()
    )
...
async def update_inv(id: int, nuevo: ActualizarInvestigacion):
    ...
    if nuevo.descripcion:
        response = (
            supabase.table("Investigacion")
            .update({"descripcion": nuevo.descripcion})
            .eq("id", id)
            .execute()
        )
        return response
...
async def upsert_producto(producto: Producto):
    ...
    existing_product = (
        supabase.table("Producto").select("id").eq("nombre", producto.nombre).execute()
    )
...
async def update_producto(id: int, nuevo: ActualizarProducto):
    ...
    if response.data:
        query = (
            supabase.table("Producto")
            .update({"cantidad_total": nuevo.cantidad_total})
            .eq("id", id)
            .execute()
        )
        return query
...
async def add_trabajando(trabaja: Trabaja):
    query = (
        supabase.table("Investigacion")
        .select("*")
        .eq("id", trabaja.id_investigacion)
        .execute()
    )
    if query.data:
        response = (
            supabase.table("Trabaja")
            .insert(
                {
        ...
        return {
            "message": f"No se encontro investigacion con id: {trabaja.id_investigacion}"
        }
...
async def get_agendamiento(email: EmailStr):
    agenda_query = (
        supabase.table("Agenda").select("*").eq("email_estudiante", email).execute()
    )
    return agenda_query
...
async def check_availability(check_agenda: CheckAgenda):
    ...
    block_query = (
        ...
        .select("id_bloque")
        .match({"id_sala": check_agenda.id_sala, "fecha": check_agenda.fecha})
        .execute()
    )
    sala_query = (
        supabase.table("Sala")
        .select("capacidad")
        .eq("id", check_agenda.id_sala)
        .execute()
    )
...
async def get_prod_by_id(id_producto: int):
    response = (
        supabase.table("Producto_en_sala")
        .select("*")
...
async def update_prod_en_sala(
    id_producto: int, id_sala: int, actualizado: ActualizarProd_en_sala
):
    if actualizado.cantidad:
        if await check_stock(id_producto, actualizado.cantidad):
            response = (
                supabase.table("Producto_en_sala")
                .update({"cantidad": actualizado.cantidad, "lugar": actualizado.lugar})
                .match({"id_producto": id_producto, "id_sala": id_sala})
                .execute()
            )
            return response
    else:
...
async def upsert_equipo(equipo: Equipo):
    # Verificar si el producto ya existe en la base de datos
    existing_equipment = (
        supabase.table("Equipo").select("id").eq("nombre", equipo.nombre).execute()
    )
    if existing_equipment.data:
        response = (
            supabase.table("Equipo")
            .upsert(
                {
...
async def get_user_by_email(email: EmailStr):
    query = (
        supabase.table("Usuario").select("email", "nombre").eq("email", email).execute()
    )
    if query.data:
        return query
    else:
...
async def get_user_scopes(email: EmailStr):
    ...
    estudiante = (
        supabase.table("Estudiante").select("email").eq("email", email).execute()
    )
    if estudiante.data:
